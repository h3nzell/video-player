import { FC, useRef, useState, KeyboardEvent, useCallback, ChangeEvent } from 'react'
...
const Player: FC = () => {
  ...
  const [currentIndicatorPosition, setCurrentIndicatorPosition] = useState<number>(0)

  const { value, preValue, isActive } = volume

  const time = videoRef.current?.currentTime
    ? secondsTimeConverterUtil(Math.floor(Number(videoRef.current?.currentTime)))
    : '---'
  ...
  const onVideoVolumeDown = useCallback(() => {
    if (!isNil(videoRef.current) && videoRef.current.volume) {
      videoRef.current.volume = Number((Number(videoRef.current.volume) - 0.05).toFixed(2))
  ...
  const onVolumeRangeHandler = useCallback((e: ChangeEvent<HTMLInputElement>) => {
  ...
  const onVideoPressHandler = (event: KeyboardEvent<HTMLVideoElement | HTMLDivElement>) => {
    switch (event.code) {
      case 'KeyM':
        onToggleMuteHandler()
        break
  ...
  const onVideoTimeUpdate = useCallback(() => {
    setCurrentVideoTime((Number(videoRef.current?.currentTime) / Number(videoRef.current?.duration)) * 100)
  }, [])

  const onTimelineMouseClickHandler = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    const rect = timelineRef.current?.getBoundingClientRect()
    const distanceFromTimelineStart = event.clientX - Number(rect?.left)

    const result = (distanceFromTimelineStart / Number(timelineRef.current?.offsetWidth)) * 100

    if (!isNil(videoRef.current)) {
      videoRef.current.currentTime = (Number(videoRef.current?.duration) * result) / 100
    }

    setCurrentVideoTime(result)
  }, [])

  const onTimelineMouseMoveHandler = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    const rect = timelineRef.current?.getBoundingClientRect()

    const distanceFromStart = event.clientX - Number(rect?.left)
    const finalDistance = Math.max(Math.min(distanceFromStart, Number(timelineRef.current?.offsetWidth)), 0)

    const result = secondsTimeConverterUtil(
      (finalDistance / Number(timelineRef.current?.offsetWidth)) * Number(videoRef.current?.duration)
    )
  ...
  return (
    <div ref={containerRef} className={classNames(styles.container, { [styles.container__full__screen]: isFullScreen })}>
      <Button
        onClick={onTogglePlayHandler}
        className={classNames(styles.container__center__play, { [styles.container__center__play__active]: !isPlay })}
      > 
        <PlayIcon />
      </Button>
        ...
        <div className={styles.container__controllers__bottom}>
          ...
          >
            <div
              ...
              className={classNames(styles.container__controllers__bottom__timeline__filled)}
            />
            <div
              style={{ left: currentIndicatorPosition }}
              className={styles.container__controllers__bottom__timeline__indicator}
            >
              {/* TODO: Need to be refactor the HTML structure */}
              <Tooltip
                title={tooltipTime}
                className={styles.container__controllers__bottom__timeline__indicator__tooltip}
              >
                <>f</>
              </Tooltip>
            </div>
